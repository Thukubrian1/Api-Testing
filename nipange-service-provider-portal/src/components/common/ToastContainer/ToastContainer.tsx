...
const Toast: React.FC<{ toast: ToastType; onRemove: (id: string) => void }> = ({
  ...
  return (
    <div
      className={`flex items-center w-full max-w-xs p-4 mb-4 rounded-lg shadow-lg border ${getColorClasses()} animate-slide-in-right`}
      role="alert"
    >
      <div className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 ${getIconColorClass()}`}>
        {getIcon()}
      </div>
